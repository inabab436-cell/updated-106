...
describe("recall_earlier_conversation freshness (all knowledge types)", () => {
  for (const c of CASES) {
    it(`${c.label}: final answer uses only the new value`, () => {
      ...
      // 1) Tool result: customer messages intact, agent facts redacted.
      const transcript = buildRecallTranscript(conversation);
      expect(transcript).not.toContain(c.oldValue);
      expect(transcript).toContain(RECALL_REDACTION_MARKER);
...
describe("redactStoreFactsFromAgentText", () => {
  it("keeps purely conversational sentences", () => {
    expect(redactStoreFactsFromAgentText("تمام يا فندم")).toBe("تمام يا فندم");
  });

  it("replaces any sentence carrying a number or store fact", () => {
    const out = redactStoreFactsFromAgentText("تمام يا فندم. السعر 250 جنيه");
    expect(out).toContain("تمام يا فندم.");
    expect(out).toContain(RECALL_REDACTION_MARKER);
    expect(out).not.toContain("250");
  });

  it("collapses consecutive redactions into a single marker", () => {
    const out = redactStoreFactsFromAgentText("السعر 250 جنيه. الشحن 40 جنيه");
    expect(out.split(RECALL_REDACTION_MARKER)).toHaveLength(2);
  });
});
